...
describe('Desafios iniciais', () => {
  ...
  describe.skip('3 - Escreva uma query que exiba os valores da coluna que representa a primary key da tabela `products`.', () => {
  ...
  describe.skip('4 - Conte quantos registros existem em `product_name` de `products`.', () => {
  ...
  describe.skip('5 - Monte uma query que exiba os dados da tabela `products` a partir do quarto registro até o décimo terceiro, incluindo tanto um quanto o outro. Obs.: não use `where` ou `order by`.', () => {
  ...
  describe.skip('6 - Exiba os dados das colunas `product_name` e `id` da tabela `products` de maneira que os resultados estejam em ordem alfabética dos nomes.', () => {
  ...
  describe.skip('7 - Mostre apenas os ids dos 5 últimos registros da tabela `products` (a ordernação deve ser baseada na coluna `id`).', () => {
